...
#define ERR_OUT_PUT_OF_ARRAY        -13  //保留

#define ERR_GET_FACE_FEATURE        -14   //提取人脸特征失败，须保证图像中有且仅有一张人脸
...
#define ERR_IMAGE_SIZE              -25   //图像太大，用于提取特征的jpg图像不能超过10M，jpg 直接注册图像不超过6M 
...
#define ERR_VISIBLE_TARGET          -54    // 未检出指定个数可见光目标*/
...
* @brief  WG输入信号 
* @param  cam相机句柄
* @param  type GPIO的输入信号类型， 目前仅支持 4:WG66 
* @param  data 输入类型为gpio时，该值表示gpio端口号。输入类型为韦根信号时，该值为韦根协议门禁卡号
* @param  usr_param 用户参数
*/
